        ...
        <article class="privacy-document">
          ...
          <header class="privacy-hero">
            ...
            <div class="privacy-summary" aria-label="Resumo da política">
              ...
              <span>Licença verificada por serviço próprio</span>
          ...
          <div class="privacy-content">
            ...
            <section><h2>2. Informações armazenadas pela extensão</h2><p>A extensão utiliza o armazenamento local do navegador chrome.storage.local para manter dados necessários ao seu funcionamento. Esses dados ficam no dispositivo do usuário, salvo quando uma ação específica de licença exigir comunicação com o serviço próprio de licenciamento.</p><ul>${storedData.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul></section>
            <section><h2>3. Verificação de licença</h2><p>Para registrar a instalação e verificar o status da licença, a extensão se comunica com um serviço próprio de licenciamento.</p><p>Nessa comunicação podem ser enviados identificadores técnicos da instalação, chave do cliente, versão da extensão, datas de teste e informações básicas do navegador necessárias para controle de ativação, prevenção de uso indevido e suporte ao funcionamento da licença.</p></section>
            ...
            <section><h2>9. Alterações nesta política</h2><p>Esta política pode ser atualizada para refletir mudanças na extensão, no serviço de licenciamento, nas permissões usadas ou em requisitos legais e de plataforma. A versão publicada nesta URL será considerada a versão vigente.</p></section>
